...
export default function PricingPage() {
  ...
  const tiers = [
    ...
    },
    {
      ...
      popular: true,
      color: "border-blue-500 bg-blue-950/10 ring-2 ring-blue-500/10"
    },
    {
      id: "prod-enterprise",
      ...
      <div className="max-w-7xl mx-auto px-6 space-y-16">
        ...
        <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3 items-stretch">
          {tiers.map((tier) => (
            <div
              key={tier.id}
              className={`relative flex flex-col justify-between p-8 rounded-2xl border transition-all hover:scale-[1.01] ${tier.color}`}
            >
              {tier.popular && (
                ...
                <ul className="space-y-4 text-xs font-semibold text-zinc-400">
                  {tier.features.map((feature, idx) => (
                    <li key={idx} className="flex items-start gap-2.5">
                      <div className="mt-0.5 p-0.5 bg-blue-500/15 text-blue-400 rounded-full flex-shrink-0">
                        <Check size={10} />
                      </div>
                      <span>{feature}</span>
                    </li>
              ...
              <div className="pt-8 space-y-4">
                <a
                  ...
                  className={`w-full flex items-center justify-center gap-1.5 py-3 px-4 rounded-xl text-xs font-bold text-center transition-colors cursor-pointer ${
                    tier.popular
                ...
                <div className="flex items-center gap-1.5 text-[9px] font-bold text-zinc-500 tracking-wider uppercase select-none">
                  <Tag size={10} />
        ...
        {/* FAQ note info */}
        <div className="max-w-3xl mx-auto bg-zinc-900/30 border border-zinc-900 p-6 rounded-2xl space-y-3 flex items-start gap-3">
          <HelpCircle size={20} className="text-zinc-500 flex-shrink-0 mt-0.5" />
          <div className="space-y-1">
            <h4 className="font-bold text-xs text-white uppercase tracking-wider">Payment Support & Activation Note</h4>
