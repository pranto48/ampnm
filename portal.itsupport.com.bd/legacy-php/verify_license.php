...
// Auto-provisioned admin licenses (admin created, no customer required)
// Format: 'LICENSE_KEY' => ['expires_at' => 'YYYY-MM-DD', 'max_devices' => N, 'product' => 'ampos']
...
try {
    ...
    // Auto-provision admin licenses if they don't exist yet
    if (isset($AUTO_PROVISION_LICENSES[$app_license_key])) {
        $checkStmt = $pdo->prepare("SELECT id FROM `licenses` WHERE license_key = ?");
        $checkStmt->execute([$app_license_key]);
        if (!$checkStmt->fetch(PDO::FETCH_ASSOC)) {
            $autoLic = $AUTO_PROVISION_LICENSES[$app_license_key];
            $insertStmt = $pdo->prepare(
                "INSERT INTO `licenses` (customer_id, product_id, license_key, status, issued_at, expires_at, max_devices, current_devices, bound_installation_id)
                 VALUES (NULL, NULL, ?, 'active', NOW(), ?, ?, 0, NULL)"
            );
            $insertStmt->execute([$app_license_key, $autoLic['expires_at'], $autoLic['max_devices']]);
            error_log("Auto-provisioned admin license: {$app_license_key}");
        }
    }
